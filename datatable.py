...
from kivy.properties import DictProperty, NumericProperty, StringProperty,BooleanProperty, ObjectProperty 
...
if __name__ == '__main__':
    datatable = requests.get('http://warehub-api.azurewebsites.net/getstudents')
    datatable = json.loads(datatable.text)
    ...
    data = {'ids':  [x[0] for x in datatable  ],
            'names':[x[1] for x in datatable ],
            'phone':[x[2] for x in datatable  ],
            'isTA': [x[3] for x in datatable ],
            'points':[x[4] for x in datatable  ]}
...
<Bucket@AccordionItem>:
    ...
    header1 = ['ids','names','phone','isTA','points']
    ...
    header2 = ['ids','names','phone','isTA','points']
    editpage.ids.the_table.add_widget(DataTable(name='edit',
                                                data=data,
                                                header_column='points',
                                                header_row=header2,
                                                editable=True))
